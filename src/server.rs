...
use smoltcp::wire::{ IpVersion, IpProtocol, IpAddress, Ipv4Cidr, Ipv4Address, EthernetAddress, Ipv4Packet, Ipv4Repr, TcpPacket, TcpRepr };
...
fn ingress_ipv4(package: &[u8]) {
    ...
    let ipv4_repr = Ipv4Repr::parse(&ipv4_packet, &checksum_caps).expect("Failed to parse IPv4 packet");  
    ...
    match ipv4_repr.next_header {
        IpProtocol::Tcp => {
            ...
            let tcp_repr = TcpRepr::parse(&tcp_packet, &IpAddress::Ipv4(src_addr), &IpAddress::Ipv4(dst_addr), &checksum_caps).expect("Failed to parse TCP packet");
        },
        IpProtocol::Udp => {
